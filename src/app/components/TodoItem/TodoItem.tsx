...
function TodoItem({ todo, handleDelete, handleComplete, handlePriority, handleEdit, filter }: TodoItemProps) {
  ...
  return (
    <li className="w-full flex pb-3 border-b border-gray-300">
      {
        edit ? (
          <input className="flex-1 pt-1 px-1 mr-2 border border-black rounded-md w-full" value={todo.text} onChange={(e) => handleEdit(todo.id, e.target.value)} />
        ) : (
          <span className="flex-1 pt-1 mr-2">
            {todo.text}
          </span>
        )
      }
